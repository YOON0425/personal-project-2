...
characterRouter.post('/characters.router.js', async (req, res) => {
  ...
  const characterMaxId = await characters.findOne().sort('-id').exec();
  ...
  const character = new character({ value, characterNum });
